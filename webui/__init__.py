...
app = Flask(__name__)
device = get_device()
model = get_pretrained_model(params_pth='./checkpoints/version1/2025-5-19-epoch50.pt',
                             device=device)
genre_list = get_first_row(file_path='./data/mini/track_lists.csv')
...
def process_by_id(song_id):
    try:
        ...
        genre_name, max_idx, _ = process(waveform_mono, model, genre_list, device)
